...
export class AddClientPackageColumnsToSessions1769719339323 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const sessionParticipantsTable = await queryRunner.getTable(
      'session_participants',
    );
    if (sessionParticipantsTable) {
      const participantsForeignKey = sessionParticipantsTable.foreignKeys.find(
        (fk) => fk.columnNames.indexOf('client_package_id') !== -1,
      );
      if (participantsForeignKey) {
        await queryRunner.dropForeignKey(
          'session_participants',
          participantsForeignKey,
        );
      }
    ...
    if (sessionsTable) {
      const sessionsForeignKey = sessionsTable.foreignKeys.find(
        (fk) => fk.columnNames.indexOf('client_package_id') !== -1,
      );
