...
export class AddGranularStudioPermissions1770100000004 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const roleKey of rolesToAssign) {
            const roles = await queryRunner.query(`SELECT id FROM roles WHERE key = $1`, [roleKey]);
        ...
        console.log('Added granular studio/room/device permissions and assigned to owners/admins.');
    }
