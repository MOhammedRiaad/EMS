...
export class AddSettingsFeatureFlags1770100000006 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const plans = await queryRunner.query(
      `SELECT id, key, features FROM plans`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const plan of plans) {
      ...
      features = features.filter(
        (f: string) =>
          !['core.branding', 'core.cancellation_policy'].includes(f),
      );
    ...
    await queryRunner.query(
      `DELETE FROM feature_flags WHERE key IN ('core.branding', 'core.cancellation_policy')`,
    );
  }
}
