...
export class AddOwnerTenantDeletePermission1770732000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        `, [newPermissionId, 'owner.tenant.delete', 'Delete Tenant', 'Permanently delete a tenant and all their data', 'owner']);

        // 2. Get the permission ID (whether new or existing)
        const permResult = await queryRunner.query(`SELECT id FROM permissions WHERE key = 'owner.tenant.delete'`);
        ...
        if (permResult.length > 0) {
            ...
            console.log(`Permission 'owner.tenant.delete' assigned to 'platform_owner'`);
        } else {
            console.error(`Failed to find permission 'owner.tenant.delete' after insertion attempt.`);
        }
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (permResult.length > 0) {
            ...
            await queryRunner.query(`DELETE FROM role_permissions WHERE permission_id = $1`, [permId]);
