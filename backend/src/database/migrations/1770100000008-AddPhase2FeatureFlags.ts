...
export class AddPhase2FeatureFlags1770100000008 implements MigrationInterface {
    ...
    const newFeatures = [
      {
        ...
        description:
          'Enables the new advanced calendar with resource views and drag-and-drop.',
        category: 'core',
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Remove from plans
    const removeFeatureFromPlan = async (
      planKey: string,
      featureKey: string,
    ) => {
      const plan = await queryRunner.query(
    ...
    for (const featureKey of featuresToRemove) {
      ...
      // Remove feature flag
      await queryRunner.query(`DELETE FROM feature_flags WHERE key = $1`, [
        featureKey,
      ]);
    }
  }
