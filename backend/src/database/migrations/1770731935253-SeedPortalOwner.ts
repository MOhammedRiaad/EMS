...
export class SeedPortalOwner1770731935253 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      await queryRunner.query(
        `ALTER TYPE "users_role_enum" ADD VALUE IF NOT EXISTS 'platform_owner'`,
      );
      await queryRunner.query(
      ...
      console.log(
        'Enum update skipped or failed (might already exist):',
        e.message,
      );
    }
    ...
    if (existingUsers.length === 0) {
      ...
      await queryRunner.query(
        `UPDATE users SET role = 'owner', password_hash = $1 WHERE id = $2`,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (existingUsers.length > 0) {
      ...
      await queryRunner.query(`UPDATE users SET active = false WHERE id = $1`, [
        userId,
      ]);
    }
  }
