...
export class TermsService {
    ...
    async create(tenantId: string, dto: CreateTermsDto): Promise<TermsOfService> {
        if (dto.isActive) {
            // Deactivate others
            await this.termsRepo.update({ tenantId, isActive: true }, { isActive: false });
        }
    ...
    async checkStatus(tenantId: string, clientId: string): Promise<{ accepted: boolean; termsId?: string }> {
    ...
    async accept(tenantId: string, clientId: string, termsId: string, ipAddress?: string, userAgent?: string): Promise<TermsAcceptance> {
        const terms = await this.termsRepo.findOne({ where: { id: termsId, tenantId } });
