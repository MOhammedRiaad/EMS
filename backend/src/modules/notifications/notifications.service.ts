...
import {
  ClientPackage,
  ClientPackageStatus,
} from '../packages/entities/client-package.entity';
import {
  WaitingListEntry,
  WaitingListStatus,
...
export class NotificationsService {
  ...
  async getActiveAnnouncementsForUser(
    ...
    const targetedRefs = activeAnnouncements.filter((a) => {
      ...
      if (a.targetType === AnnouncementTargetType.CLIENTS && role === 'client')
        return true;
      if (a.targetType === AnnouncementTargetType.COACHES && role === 'coach')
        return true;
  ...
  async getDashboardNotifications(
    ...
    // 0. Pending Time-Off Requests
    const timeOffRequests = await this.coachesService.getTimeOffRequests(
      tenantId,
      'pending' as any,
    );
    ...
    if (timeOffRequests.length > 0) {
      notifications.push({
        ...
        title: `${timeOffRequests.length} Pending Time-Off Request${timeOffRequests.length > 1 ? 's' : ''}`,
        message: 'Review pending requests from coaches',
        link: '/admin/time-off',
        priority: 'high',
      });
    }
  ...
  async getClientNotifications(
    ...
    const lowBalance = packages.filter(
      (p) => p.sessionsRemaining <= 2 && p.sessionsRemaining > 0,
    );
