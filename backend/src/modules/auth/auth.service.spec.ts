...
describe('AuthService', () => {
    ...
    describe('login', () => {
        ...
        it('should successfully login with valid credentials', async () => {
            ...
            const result = await service.login({ ...loginDto, tenantId: 'tenant-123' }) as any;
        ...
        it('should throw UnauthorizedException with invalid password', async () => {
            ...
            await expect(service.login({ ...loginDto, tenantId: 'tenant-123' })).rejects.toThrow(
        ...
        it('should throw UnauthorizedException for ambiguous email without tenantId', async () => {
            mockUserRepository.findOne.mockResolvedValue(null); // No user with tenantId
            mockUserRepository.find.mockResolvedValue([mockUser, { ...mockUser, id: 'user-456' }]);
        ...
        it('should increment failed attempts on invalid password', async () => {
            ...
            await expect(service.login({ ...loginDto, tenantId: 'tenant-123' })).rejects.toThrow(UnauthorizedException);
        ...
        it('should block login if account is locked', async () => {
            ...
            await expect(service.login({ ...loginDto, tenantId: 'tenant-123' })).rejects.toThrow('Account locked');
        });
        ...
        it('should require 2FA if enabled', async () => {
            ...
            const result = await service.login({ ...loginDto, tenantId: 'tenant-123' });
    ...
    describe('createUser', () => {
        ...
        it('should throw ForbiddenException if non-admin tries to create user', async () => {
            ...
            await expect(service.createUser(createDto, clientUser)).rejects.toThrow(ForbiddenException);
        });

        it('should throw ForbiddenException if coach tries to create user', async () => {
            const coachUser = { ...mockUser, role: 'coach' } as User;

            await expect(service.createUser(createDto, coachUser)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('createClientUser', () => {
        ...
        it('should throw UnauthorizedException if email exists in tenant', async () => {
            ...
            await expect(service.createClientUser(createDto, 'tenant-123')).rejects.toThrow(
                UnauthorizedException
            );
        });
    });
    ...
    describe('generateTokens', () => {
        ...
        it('should include clientId in payload if user has client', () => {
            const userWithClient = { ...mockUser, client: { id: 'client-123' } } as User;
            mockJwtService.sign.mockReturnValue('mock-token');
    ...
    describe('findByEmail', () => {
        ...
        it('should return null if user not found', async () => {
            ...
            const result = await service.findByEmail('nonexistent@example.com', 'tenant-123');
    ...
    describe('setupPassword', () => {
        ...
        it('should setup password with valid invite token', async () => {
            ...
            mockUserRepository.findOne.mockResolvedValue(mockUser);
            mockUserRepository.save.mockResolvedValue({ ...mockUser, emailVerified: true, active: true });
            mockTenantsService.findOne.mockResolvedValue(mockTenant);
        ...
        it('should throw UnauthorizedException with invalid token', async () => {
            ...
            await expect(service.setupPassword(setupDto)).rejects.toThrow(UnauthorizedException);
            await expect(service.setupPassword(setupDto)).rejects.toThrow('Invalid or expired invitation token');
        });
        ...
        it('should throw UnauthorizedException with non-invite token type', async () => {
            ...
            await expect(service.setupPassword(setupDto)).rejects.toThrow(UnauthorizedException);
            await expect(service.setupPassword(setupDto)).rejects.toThrow('Invalid token type');
        });
        ...
        it('should mark user as email verified and active', async () => {
            ...
            mockJwtService.verify.mockReturnValue(tokenPayload);
            mockUserRepository.findOne.mockResolvedValue({ ...mockUser, emailVerified: false, active: false });
            mockTenantsService.findOne.mockResolvedValue(mockTenant);
    ...
    describe('2FA Flows', () => {
        ...
        it('should verify 2FA login and return tokens', async () => {
            ...
            const result = await service.verifyTwoFactorLogin(userWith2FA.id, 'valid-token');
            expect(result).toHaveProperty('accessToken');
        });
    });
    ...
    describe('Password Reset', () => {
        ...
        it('should reset password with valid token', async () => {
            ...
            const result = await service.resetPassword(mockUser.email, 'valid-token', 'newPass');
            expect(result).toHaveProperty('message');
            expect(mockUserRepository.save).toHaveBeenCalled();
        });
    });
