...
export class AnalyticsService {
    ...
    async getRevenueSummary(tenantId: string) {
        ...
        const growthPercent = lastMonth > 0 ? ((mtd - lastMonth) / lastMonth) * 100 : 0;
    ...
    async getClientRetention(tenantId: string) {
        ...
        const activePrevious30Days = parseInt(activePrevious30Result?.count || '0', 10);
        ...
        const retentionRate = activePrevious30Days > 0
            ? (retained / activePrevious30Days) * 100
            : 0;
    ...
    async getRoomUtilization(tenantId: string, query: DateRangeQueryDto) {
        ...
        const sessionMap = new Map(sessionCounts.map(r => [r.roomId, parseInt(r.sessionCount || '0', 10)]));
        ...
        return rooms.map(room => {
            ...
            const utilization = maxSlotsPerRoom > 0 ? (sessions / maxSlotsPerRoom) * 100 : 0;
            return {
                roomId: room.id,
                roomName: room.name,
    ...
    async getDeviceUtilization(tenantId: string, query: DateRangeQueryDto) {
        ...
        const sessionMap = new Map(sessionCounts.map(r => [r.deviceId, parseInt(r.sessionCount || '0', 10)]));
        ...
        return devices.map(device => {
            ...
            const utilization = maxSlotsPerDevice > 0 ? (sessions / maxSlotsPerDevice) * 100 : 0;
            return {
                deviceId: device.id,
    ...
    async getSessionStats(tenantId: string, query: DateRangeQueryDto) {
        ...
        const scheduled = await this.sessionRepo.count({
            where: { tenantId, status: 'scheduled', startTime: MoreThanOrEqual(new Date()) },
        });
