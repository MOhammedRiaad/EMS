...
export class WaiversService {
  ...
  async getClientSignatureStatus(
    tenantId: string,
    userId: string,
  ): Promise<{ signed: boolean; signedAt?: Date; waiverId?: string }> {
  ...
  async signWaiver(
    ...
    const waiver = await this.waiverRepository.findOne({
      where: { id: dto.waiverId, tenantId },
    });
    if (!waiver) {
  ...
  async getSignedWaiversForClient(
    tenantId: string,
    clientId: string,
  ): Promise<ClientWaiver[]> {
