...
describe('CoachPortalService', () => {
    ...
    describe('getSchedule', () => {
        it('should return coach schedule for day', async () => {
            ...
            const result = await service.getSchedule('user-123', 'tenant-123', new Date(), 'day');
        ...
        it('should return schedule for week', async () => {
            ...
            const result = await service.getSchedule('user-123', 'tenant-123', new Date(), 'week');
        ...
        it('should return future sessions', async () => {
            ...
            const result = await service.getSchedule('user-123', 'tenant-123', new Date(), 'future');
    ...
    describe('updateSessionStatus', () => {
        it('should update session status', async () => {
            ...
            sessionsRepo.save.mockResolvedValue({ ...mockSession, status: 'completed' as SessionStatus });

            const result = await service.updateSessionStatus('session-123', 'user-123', 'tenant-123', 'completed');
        ...
        it('should throw NotFoundException if session not found', async () => {
            ...
            await expect(
                service.updateSessionStatus('session-123', 'user-123', 'tenant-123', 'completed')
    ...
    describe('getAvailability', () => {
        it('should return coach availability rules', async () => {
            coachesRepo.findOne.mockResolvedValueOnce(mockCoach).mockResolvedValueOnce(mockCoach);
    ...
    describe('updateAvailability', () => {
        const newRules = [{ dayOfWeek: 1, available: true, startTime: '09:00', endTime: '17:00' }];

        it('should update coach availability rules', async () => {
            coachesRepo.findOne.mockResolvedValue(mockCoach);
            coachesRepo.save.mockResolvedValue({ ...mockCoach, availabilityRules: newRules });
        ...
        it('should throw NotFoundException if coach not found', async () => {
            coachesRepo.findOne.mockResolvedValueOnce(mockCoach).mockResolvedValueOnce(null);
