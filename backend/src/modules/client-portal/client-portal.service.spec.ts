...
describe('ClientPortalService', () => {
  ...
  describe('bookSession', () => {
    ...
    it('should find default studio if not provided', async () => {
      ...
      expect(sessionsService.findFirstActiveStudio).toHaveBeenCalledWith(
        'tenant-123',
      );
    });
  });
  ...
  describe('cancelSession', () => {
    beforeEach(() => {
      ...
      sessionsService.updateStatus.mockResolvedValue({
        ...mockSession,
        status: 'cancelled',
      } as any);
    });
  ...
  describe('getMyWaitingList', () => {
    it('should return formatted waiting list entries for client', async () => {
      waitingListService.findByClient.mockResolvedValue([
        mockWaitingEntry,
      ] as any);
      ...
      expect(waitingListService.findByClient).toHaveBeenCalledWith(
        'client-123',
        'tenant-123',
      );
    });
  ...
  describe('cancelWaitingListEntry', () => {
    it('should remove waiting list entry', async () => {
      ...
      const result = await service.cancelWaitingListEntry(
        'client-123',
        'tenant-123',
        'wait-123',
      );

      expect(waitingListService.remove).toHaveBeenCalledWith(
        'wait-123',
        'tenant-123',
      );
  ...
  describe('getCoaches', () => {
    ...
    it('should return available coaches', async () => {
      ...
      expect(coachesService.findActive).toHaveBeenCalledWith(
        'tenant-123',
        'male',
        'studio-123',
      );
  ...
  describe('toggleFavoriteCoach', () => {
    it('should favorite a coach if not already favorited', async () => {
      ...
      const result = await service.toggleFavoriteCoach(
        'client-123',
        'tenant-123',
        'coach-123',
      );
  ...
  describe('getFavoriteCoaches', () => {
    it('should return list of favorite coaches', async () => {
      ...
      const result = await service.getFavoriteCoaches(
        'client-123',
        'tenant-123',
      );
