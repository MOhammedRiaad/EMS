...
export class ClientPortalService {
  ...
  async getDashboard(clientId: string, tenantId: string) {
    ...
    if (!displayPackage) {
      const packages = await this.packagesService.getClientPackages(
        clientId,
        tenantId,
      );
      displayPackage =
        packages.find((p) => p.status === ClientPackageStatus.ACTIVE) ||
        packages[0] ||
        null;
    }
  ...
  async joinWaitingList(
    ...
    let studioId: string | undefined =
      dto.studioId || client?.studioId || undefined;
    if (!studioId) {
      const firstStudio =
        await this.sessionsService.findFirstActiveStudio(tenantId);
  ...
  async updateProfile(
    ...
    if ((dto as any).privacyPreferences !== undefined)
      updateData.privacyPreferences = (dto as any).privacyPreferences;
    if ((dto as any).consentFlags !== undefined)
      updateData.consentFlags = (dto as any).consentFlags;
    ...
    if (Object.keys(updateData).length > 0) {
      clientUpdated = await this.clientsService.update(
        clientId,
        updateData,
        tenantId,
      );
    ...
    if (dto.gender && client.userId) {
      await this.authService.update(client.userId, {
  ...
  async getMyWaitingList(clientId: string, tenantId: string) {
    const entries = await this.waitingListService.findByClient(
      clientId,
      tenantId,
    );
    ...
    return entries.map((entry) => ({
      ...
      status: entry.status,
      studio: entry.studio
        ? { id: entry.studio.id, name: entry.studio.name }
        : null,
  ...
  async deleteProgressPhoto(
    ...
    const photo = await this.progressPhotoRepo.findOne({
