...
export class ClientPortalController {
  ...
  async cancelSession(
    ...
    if (!clientId)
      throw new UnauthorizedException('User is not linked to a client profile');
    return this.clientPortalService.cancelSession(
      clientId,
      tenantId,
      id,
      reason,
  ...
  async cancelWaitingListEntry(@Request() req: any, @Param('id') id: string) {
    ...
    if (!clientId)
      throw new UnauthorizedException('User is not linked to a client profile');
    return this.clientPortalService.cancelWaitingListEntry(
      clientId,
      tenantId,
      id,
    );
  ...
  async toggleFavoriteCoach(
    ...
    if (!clientId)
      throw new UnauthorizedException('User is not linked to a client profile');
    return this.clientPortalService.toggleFavoriteCoach(
      clientId,
      tenantId,
      coachId,
    );
  ...
  @Get('account')
  @ApiOperation({
    summary: 'Get account details with notification preferences',
  })
