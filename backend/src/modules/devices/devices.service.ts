...
export class DevicesService {
    ...
    async findAvailableByStudio(studioId: string, tenantId: string): Promise<EmsDevice[]> {
        return this.deviceRepository.find({
    ...
    async update(id: string, dto: UpdateDeviceDto, tenantId: string): Promise<EmsDevice> {
        const device = await this.findOne(id, tenantId);
        Object.assign(device, dto);
        return this.deviceRepository.save(device);
    }

    async updateStatus(id: string, status: EmsDevice['status'], tenantId: string): Promise<EmsDevice> {
        const device = await this.findOne(id, tenantId);
