...
describe('TenantsService', () => {
    ...
    describe('findBySlug', () => {
        it('should return tenant by slug', async () => {
            ...
            expect(result).toBe(mockTenant);
            expect(repository.findOne).toHaveBeenCalledWith({ where: { slug: 'test-tenant' } });
        });
    ...
    describe('update', () => {
        it('should update tenant', async () => {
            repository.findOne.mockResolvedValue(mockTenant);
            repository.save.mockResolvedValue({ ...mockTenant, name: 'Updated Name' });

            const result = await service.update('tenant-123', { name: 'Updated Name' });
