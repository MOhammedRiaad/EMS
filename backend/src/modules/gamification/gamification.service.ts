...
import { ClientAchievement, AchievementType } from './entities/client-achievement.entity';
...
export class GamificationService {
    ...
    async getLeaderboard(tenantId: string, currentClientId?: string): Promise<LeaderboardEntryDto[]> {
        const raw = await this.clientRepo
            .createQueryBuilder('client')
            .leftJoin('client.sessions', 'session', 'session.status = :status', { status: 'completed' })
            .select([
                'client.id',
                'client.firstName',
                'client.lastName',
            ...
            .andWhere(`(client.privacy_preferences IS NULL OR client.privacy_preferences->>'leaderboard_visible' != 'false')`)
            .groupBy('client.id')
    ...
    async getActivityFeed(tenantId: string): Promise<any[]> {
        ...
        // Sort combined
        return feedItems.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 20);
    }
    ...
    async setGoal(clientId: string, tenantId: string, dto: CreateGoalDto) {
        ...
        if (goal) {
            ...
            goal = this.goalRepo.create({
                ...
                goalType,
                targetValue,
                deadline: deadline ? new Date(deadline) : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000), // Default 90 days
                notes,
                status: GoalStatus.ACTIVE,
                startValue: null,
            });
    ...
    private async checkReviewAchievements(clientId: string, tenantId: string) {
        ...
        if (reviewCount >= 1) {
            await this.unlockAchievement(clientId, tenantId, AchievementType.FIRST_REVIEW);
        }
        if (reviewCount >= 5) {
            await this.unlockAchievement(clientId, tenantId, AchievementType.REVIEWS_5);
        }
    }

    private async unlockAchievement(clientId: string, tenantId: string, type: AchievementType) {
