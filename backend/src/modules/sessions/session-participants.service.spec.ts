...
describe('SessionParticipantsService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<SessionParticipantsService>(
      SessionParticipantsService,
    );
  ...
  describe('addParticipant', () => {
    it('should add a participant to a group session', async () => {
      ...
      mockPackagesService.findBestPackageForSession.mockResolvedValue(
        activePackage,
      );
    ...
    it('should fail if session is individual', async () => {
      ...
      await expect(service.addParticipant('s1', 'c1', 't1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('updateStatus', () => {
    ...
    it('should refund session when status changes from completed to cancelled', async () => {
      ...
      expect(mockPackagesService.returnSession).toHaveBeenCalledWith(
        'p1',
        't1',
      );
