...
describe('LeadService', () => {
    ...
    describe('convertToClient', () => {
        it('should convert lead to client, invite, and update status', async () => {
            leadRepository.findOne.mockResolvedValue(mockLead);
            clientsService.createWithUser.mockResolvedValue({ id: 'client-123' } as any);
            ...
            expect(clientsService.invite).toHaveBeenCalledWith('client-123', mockUser.tenantId);

            expect(leadRepository.save).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should throw BadRequest if lead already converted', async () => {
            const convertedLead = { ...mockLead, status: LeadStatus.CONVERTED } as Lead;
            leadRepository.findOne.mockResolvedValue(convertedLead);

            await expect(service.convertToClient('lead-123', mockUser))
