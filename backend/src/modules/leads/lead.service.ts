...
export class LeadService {
    ...
    async create(createLeadDto: any, tenantId: string, user?: User): Promise<Lead> {
        ...
        if (savedLead.id) {
            await this.logActivity(savedLead.id, LeadActivityType.NOTE, 'Lead created', user);
            // Trigger Automation
            await this.automationService.triggerEvent(AutomationTriggerType.NEW_LEAD, { lead: savedLead });
        }
    ...
    async findAll(filter: any, tenantId: string): Promise<Lead[]> {
        ...
        if (filter.search) {
            query.andWhere('(lead.firstName ILIKE :search OR lead.lastName ILIKE :search OR lead.email ILIKE :search)', { search: `%${filter.search}%` });
        }
    ...
    private async logActivity(leadId: string, type: LeadActivityType, content: string, user?: User) {
    ...
    async convertToClient(id: string, user?: User): Promise<any> {
        ...
        await this.logActivity(id, LeadActivityType.STATUS_CHANGED, 'Lead converted to Client', user);
