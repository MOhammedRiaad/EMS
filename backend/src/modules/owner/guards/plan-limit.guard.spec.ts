...
describe('PlanLimitGuard', () => {
    ...
    beforeEach(async () => {
        ...
        usageTrackingService = module.get(UsageTrackingService) as jest.Mocked<UsageTrackingService>;
    });
    ...
    it('should allow access if limit is not exceeded', async () => {
        ...
        expect(result).toBe(true);
        expect(usageTrackingService.checkLimit).toHaveBeenCalledWith('t1', 'clients');
    });
});
