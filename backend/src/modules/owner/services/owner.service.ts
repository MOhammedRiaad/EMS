...
export class OwnerService {
    ...
    async getDashboardStats(): Promise<DashboardStats> {
        ...
        const [
            ...
            this.sessionRepository.count({ where: { startTime: MoreThanOrEqual(startOfDay) } }),
            this.sessionRepository.count({ where: { startTime: MoreThanOrEqual(startOfMonth) } }),
        ]);
     ...
     */
    async listTenants(filters: TenantListFilters): Promise<{ items: any[]; total: number }> {
        const queryBuilder = this.tenantRepository.createQueryBuilder('tenant');

        if (filters.search) {
            queryBuilder.andWhere('(tenant.name ILIKE :search OR tenant.slug ILIKE :search)', {
    ...
    async getTenantDetails(tenantId: string): Promise<any> {
        ...
        const [usage, features, owner] = await Promise.all([
            ...
            this.userRepository.findOne({
                ...
                select: ['id', 'email', 'firstName', 'lastName', 'phone', 'lastLoginAt'],
            }),
        ]);
        ...
        try {
            ...
            planDetails.name = tenant.plan.charAt(0).toUpperCase() + tenant.plan.slice(1);
        }
     ...
     */
    async reactivateTenant(tenantId: string, ownerId: string, ipAddress?: string): Promise<Tenant> {
        const tenant = await this.tenantRepository.findOne({ where: { id: tenantId } });
        if (!tenant) {
            throw new NotFoundException(`Tenant ${tenantId} not found`);
        }
    ...
    async updateTenantPlan(
        ...
        const updatedTenant = await this.planService.assignPlanToTenant(tenantId, planKey);
     ...
     */
    async resetDemoData(tenantId: string, ownerId: string, ipAddress?: string): Promise<void> {
        const tenant = await this.tenantRepository.findOne({ where: { id: tenantId } });
        if (!tenant) {
            throw new NotFoundException(`Tenant ${tenantId} not found`);
        }
        ...
        await this.auditService.logAction(ownerId, 'RESET_DEMO_DATA', {}, tenantId, ipAddress);
    ...
    > {
        ...
        const results: Array<{ tenant: Tenant; limitType: string; percentage: number }> = [];
        ...
        for (const tenant of tenants) {
            ...
            const checkLimit = (item: { percentage: number }, type: string) => {
                if (item.percentage >= threshold) {
                    results.push({ tenant, limitType: type, percentage: item.percentage });
                }
            };
     ...
     */
    async deleteTenant(tenantId: string, ownerId: string, ipAddress?: string): Promise<void> {
        const tenant = await this.tenantRepository.findOne({ where: { id: tenantId } });
        if (!tenant) {
            throw new NotFoundException(`Tenant ${tenantId} not found`);
        }
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            await manager.query(`DELETE FROM client_session_reviews WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM session_participants WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM waiting_list WHERE "tenant_id" = $1`, [tenantId]);

            // Sessions must be deleted before clients/coaches because of RESTRICT
            await manager.query(`DELETE FROM sessions WHERE "tenant_id" = $1`, [tenantId]);

            // 2. Delete Client related data
            await manager.query(`DELETE FROM client_goals WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM client_achievements WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM client_waivers WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM client_progress_photos WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM inbody_scans WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM terms_acceptance WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM parq_responses WHERE "tenant_id" = $1`, [tenantId]);
            // Client packages usually link to packages
            await manager.query(`DELETE FROM client_packages WHERE "tenant_id" = $1`, [tenantId]);

            // 3. Transactions and Sales
            await manager.query(`DELETE FROM sale_items WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM sales WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM transactions WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM product_stocks WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM products WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM packages WHERE "tenant_id" = $1`, [tenantId]);

            // 4. Marketing / Leads
            await manager.query(`DELETE FROM lead_activities WHERE lead_id IN (SELECT id FROM leads WHERE "tenant_id" = $1)`, [tenantId]);
            await manager.query(`DELETE FROM leads WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM automation_executions WHERE "tenantId" = $1`, [tenantId]);
            await manager.query(`DELETE FROM automation_rules WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM notification_preferences WHERE "tenantId" = $1`, [tenantId]); // If table exists? No, it's a column on user. 
            // Check notifications table
            await manager.query(`DELETE FROM notifications WHERE "tenantId" = $1`, [tenantId]);

            // 5. Core Entities
            // Clients must be deleted before users?
            // Users have cascade from Tenant, but Clients might reference Users.
            // Delete Clients first.
            await manager.query(`DELETE FROM clients WHERE "tenant_id" = $1`, [tenantId]);

            // delete favorite_coaches
            await manager.query(`DELETE FROM favorite_coaches WHERE "tenant_id" = $1`, [tenantId]);

            // Coaches
            await manager.query(`DELETE FROM coaches WHERE "tenant_id" = $1`, [tenantId]);
            ...
            await manager.query(`DELETE FROM audit_logs WHERE "tenantId" = $1`, [tenantId]);

            // Finally Users
            await manager.query(`DELETE FROM users WHERE "tenant_id" = $1`, [tenantId]);

            // Studios, Rooms, Devices
            await manager.query(`DELETE FROM ems_devices WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM rooms WHERE "tenant_id" = $1`, [tenantId]);
            await manager.query(`DELETE FROM studios WHERE "tenant_id" = $1`, [tenantId]);
     ...
     */
    async anonymizeTenant(tenantId: string, ownerId: string, ipAddress?: string): Promise<Tenant> {
        const tenant = await this.tenantRepository.findOne({ where: { id: tenantId } });
        if (!tenant) {
            throw new NotFoundException(`Tenant ${tenantId} not found`);
        }
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            for (const client of clients) {
                const anonId = uuidv4();
                await manager.update(Client, { id: client.id }, {
                    firstName: 'Anonymized',
                    lastName: 'Client',
                    email: client.email ? `anonymized_client_${anonId}@deleted.local` : null,
                    phone: null,
                    dateOfBirth: null,
                    healthNotes: null,
                    notes: null,
                    medicalHistory: { allergies: [], injuries: [], conditions: [], custom: {} },
                    avatarUrl: null,
                    status: 'inactive'
                });
            }
            ...
            for (const coach of coaches) {
                await manager.update(Coach, { id: coach.id }, {
                    bio: 'Anonymized',
                    active: false
                });
            }
        ...
        return this.tenantRepository.findOne({ where: { id: tenantId } }) as Promise<Tenant>;
    }
    ...
    async getComplianceStats() {
        ...
        const activeClientsCount = await this.clientRepository.count({ where: { status: In(['active', 'suspended']) } });
        ...
        const anonymizedTenantsCount = await this.auditService.countActions('ANONYMIZE_TENANT' as any);
        const deletedTenantsCount = await this.auditService.countActions('DELETE_TENANT' as any);
        ...
        return {
            clients: {
                ...
                marketingConsentRate: activeClientsCount > 0 ? Math.round((parseInt(marketingConsentCount[0].count) / activeClientsCount) * 100) : 0,
                dataProcessingConsentRate: activeClientsCount > 0 ? Math.round((parseInt(dataProcessingConsentCount[0].count) / activeClientsCount) * 100) : 0,
                termsAcceptanceRate: activeClientsCount > 0 ? Math.round((parseInt(termsAcceptedCount.count) / activeClientsCount) * 100) : 0,
            },
            rightToBeForgotten: {
