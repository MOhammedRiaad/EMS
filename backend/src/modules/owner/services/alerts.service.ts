...
export class AlertsService {
  ...
  private readonly thresholds: AlertThreshold[] = [
    {
      ...
      messageTemplate:
        'Tenant {tenantName} has used {percentage}% of their client limit',
    },
    {
      type: 'usage_at_limit',
      metric: 'clients',
    ...
    },
    {
      ...
      messageTemplate:
        'Tenant {tenantName} has used {percentage}% of their monthly session limit',
    },
    {
      type: 'trial_expiring',
      metric: 'trialDays',
  ...
  async checkUsageThresholds(): Promise<void> {
    ...
    try {
      const tenantsApproaching =
        await this.ownerService.getTenantsApproachingLimits(80);
      ...
      this.logger.log(
        `Usage threshold check complete. Found ${tenantsApproaching.length} tenants approaching limits.`,
      );
    } catch (error) {
  ...
  async checkInactiveTenants(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Inactive tenant check complete. Found ${inactiveTenants.length} inactive tenants.`,
      );
    } catch (error) {
  ...
  private createUsageAlert(
    ...
    const alertType =
      percentage >= 100 ? 'usage_at_limit' : 'usage_approaching_limit';
