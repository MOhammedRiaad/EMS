...
describe('OwnerService', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(User), useValue: { count: jest.fn(), findOne: jest.fn(), find: jest.fn() } },
                { provide: getRepositoryToken(Session), useValue: { count: jest.fn() } },
                { provide: getRepositoryToken(Client), useValue: { count: jest.fn() } },
                { provide: getRepositoryToken(Coach), useValue: { count: jest.fn() } },
                { provide: getRepositoryToken(TermsAcceptance), useValue: { createQueryBuilder: jest.fn() } },
                {
                    provide: UsageTrackingService,
                    useValue: { getUsageSnapshot: jest.fn(), getGlobalUsageStats: jest.fn() },
                },
                { provide: FeatureFlagService, useValue: { getFeaturesForTenant: jest.fn() } },
                { provide: PlanService, useValue: { assignPlanToTenant: jest.fn(), getPlanByKey: jest.fn() } },
                { provide: OwnerAuditService, useValue: { logAction: jest.fn(), countActions: jest.fn() } },
    ...
    describe('suspendTenant', () => {
        ...
        it('should throw NotFoundException if tenant does not exist', async () => {
            ...
            await expect(service.suspendTenant('m1', 'r', 'o'))
    ...
    describe('listTenants', () => {
        it('should return flattened tenant DTOs with plan objects and contact emails', async () => {
            const mockTenants = [
                { id: 't1', name: 'Tenant 1', plan: 'pro', usageStats: { clients: 5 }, createdAt: new Date() },
                { id: 't2', name: 'Tenant 2', plan: 'starter', usageStats: null, createdAt: new Date() }
            ] as Tenant[];
            ...
            expect(result.items[1].stats).toEqual({ clients: 0, sessionsThisMonth: 0 }); // Fallback
        });
    });

    describe('getTenantDetails', () => {
        it('should return flattened detail DTO with plan, owner, and usage info', async () => {
            const mockTenantDetails = { id: 't1', name: 'Tenant 1', plan: 'pro' } as Tenant;
            ...
            (featureService.getFeaturesForTenant as jest.Mock).mockResolvedValue(mockFeatures);
        ...
        it('should handle missing plan details gracefully', async () => {
            ...
            (planService.getPlanByKey as jest.Mock).mockRejectedValue(new Error('Not found'));
