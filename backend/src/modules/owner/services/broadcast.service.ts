...
export class BroadcastService {
    ...
    async create(createDto: Partial<BroadcastMessage>, ownerId: string): Promise<BroadcastMessage> {
    ...
    async getHistory(): Promise<BroadcastMessage[]> {
        return this.broadcastRepo.find({
            where: {
                status: In([BroadcastStatus.SENT, BroadcastStatus.FAILED, BroadcastStatus.SCHEDULED]),
            },
            order: { createdAt: 'DESC' },
        });
    }

    async send(id: string, ownerId: string, ip: string): Promise<BroadcastMessage> {
        const broadcast = await this.broadcastRepo.findOne({ where: { id } });
        if (!broadcast) throw new BadRequestException('Broadcast not found');
        if (broadcast.status === BroadcastStatus.SENT) throw new BadRequestException('Already sent');
        ...
        await this.auditService.logAction(
            ownerId,
            'BROADCAST_MESSAGE',
            { broadcastId: broadcast.id, subject: broadcast.subject, audience: broadcast.targetAudience },
            undefined,
            ip
        );
    ...
    private async processBroadcast(broadcast: BroadcastMessage) {
        ...
        switch (broadcast.targetAudience) {
            ...
            case BroadcastAudience.TENANT_OWNERS:
                ...
                const owners = await this.userRepo.createQueryBuilder('user')
                    .innerJoinAndSelect('user.roles', 'role', 'role.name = :roleName', { roleName: 'tenant_owner' })
                    .getMany();
                recipients = owners
                    .filter(u => u.email)
            ...
            case BroadcastAudience.ALL_CLIENTS:
                const clients = await this.clientRepo.find({ select: ['email', 'firstName', 'lastName'] });
                recipients = clients
                    .filter(c => c.email)
        ...
        this.logger.log(`Sending broadcast ${broadcast.id} to ${recipients.length} recipients`);
