...
export class UsageTrackingService {
    ...
    async getUsageSnapshot(tenantId: string): Promise<UsageSnapshot> {
        ...
        // Get current counts
        const clientCount = await this.clientRepository.count({ where: { tenantId } });
        const coachCount = await this.coachRepository.count({ where: { tenantId, active: true } });
        ...
        return {
            ...
            sessionsThisMonth: this.calculateUsageItem(sessionsThisMonth, limits.maxSessionsPerMonth),
            smsThisMonth: this.calculateUsageItem(smsThisMonth, limits.smsAllowance),
            emailThisMonth: this.calculateUsageItem(emailThisMonth, limits.emailAllowance),
    ...
    async checkLimit(
        ...
        switch (limitType) {
            case 'clients':
                return checkItem(snapshot.clients, 'clients', 'Client limit reached. Upgrade your plan to add more clients.');
            case 'coaches':
                return checkItem(snapshot.coaches, 'coaches', 'Coach limit reached. Upgrade your plan to add more coaches.');
            case 'sessions':
                return checkItem(snapshot.sessionsThisMonth, 'sessions', 'Monthly session limit reached. Upgrade your plan to book more sessions.');
            case 'sms':
                return checkItem(snapshot.smsThisMonth, 'sms', 'SMS allowance exhausted. Upgrade your plan to send more SMS messages.');
            case 'email':
                return checkItem(snapshot.emailThisMonth, 'email', 'Email allowance exhausted. Upgrade your plan to send more emails.');
            case 'storage':
                return checkItem(snapshot.storageGB, 'storage', 'Storage limit reached. Upgrade your plan for more storage.');
            default:
                return null;
        }
    }
    ...
    async getGlobalUsageStats(): Promise<{
        ...
        const [totalClients, totalCoaches, totalSessionsThisMonth] = await Promise.all([
    ...
    // Helper methods
    private calculateUsageItem(current: number, limit: number): { current: number; limit: number; percentage: number } {
        if (limit === -1) {
    ...
    private async getMonthlyMetric(tenantId: string, metricType: string): Promise<number> {
        const now = new Date();
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        ...
        const result = await this.usageMetricRepository
            ...
            .andWhere('metric.date BETWEEN :start AND :end', { start: startOfMonth, end: endOfMonth })
            .getRawOne();

        return parseInt(result?.total || '0', 10);
    }

    private async getLatestMetric(tenantId: string, metricType: string): Promise<number> {
