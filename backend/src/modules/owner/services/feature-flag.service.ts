...
export class FeatureFlagService {
  ...
  async getFeaturesForTenant(
    ...
    Array<{ feature: FeatureFlag; enabled: boolean; source: string }>
  > {
    const allFeatures = await this.featureFlagRepository.find({
      order: { category: 'ASC', key: 'ASC' },
    });
    const overrides = await this.featureAssignmentRepository.find({
      where: { tenantId },
    });
    ...
    const plan = tenant
      ? await this.planRepository.findOne({ where: { key: tenant.plan } })
      : null;
      ...
      for (const depKey of feature.dependencies) {
        ...
        if (!depEnabled) {
          throw new BadRequestException(
            `Cannot enable "${featureKey}": dependency "${depKey}" is not enabled`,
          );
        }
      }
    }
