...
export class PlanService {
    ...
    async comparePlans(): Promise<{
        ...
        for (const plan of plans) {
            comparison[plan.key] = {
                maxClients: plan.limits.maxClients === -1 ? 'Unlimited' : plan.limits.maxClients,
                maxCoaches: plan.limits.maxCoaches === -1 ? 'Unlimited' : plan.limits.maxCoaches,
                maxSessionsPerMonth: plan.limits.maxSessionsPerMonth === -1 ? 'Unlimited' : plan.limits.maxSessionsPerMonth,
                smsAllowance: plan.limits.smsAllowance === -1 ? 'Unlimited' : plan.limits.smsAllowance,
                emailAllowance: plan.limits.emailAllowance === -1 ? 'Unlimited' : plan.limits.emailAllowance,
                storageGB: plan.limits.storageGB === -1 ? 'Unlimited' : `${plan.limits.storageGB} GB`,
