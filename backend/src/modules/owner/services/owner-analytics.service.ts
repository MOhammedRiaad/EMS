...
export class OwnerAnalyticsService {
     ...
     */
    async getGlobalAnalytics(startDate?: Date, endDate?: Date): Promise<GlobalAnalytics> {
        const now = new Date();
    ...
    async getRevenueAnalytics(startDate: Date, endDate: Date): Promise<RevenueAnalytics> {
        try {
            ...
            const averageRevenuePerTenant = tenantCount > 0 ? totalRevenue / tenantCount : 0;
    ...
    async getUsageTrends(startDate: Date, endDate: Date): Promise<UsageTrends> {
        try {
            ...
            const tenantCount = await this.tenantRepository.count();
            const avgSessionsPerTenant = tenantCount > 0 ? totalSessions / tenantCount : 0;
     ...
     */
    private async getRevenueByPeriod(months: number): Promise<{ date: string; amount: number }[]> {
     ...
     * Get sessions by day
     */
    private async getSessionsByDay(startDate: Date, endDate: Date): Promise<{ date: string; count: number }[]> {
        try {
