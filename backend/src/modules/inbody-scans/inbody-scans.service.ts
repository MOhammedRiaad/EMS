...
export class InBodyScansService {
    ...
    async findAll(query: InBodyScanQueryDto, tenantId: string): Promise<InBodyScan[]> {
        ...
        if (query.startDate && query.endDate) {
            where.scanDate = Between(new Date(query.startDate), new Date(query.endDate));
        }
    ...
    async findByClient(clientId: string, tenantId: string): Promise<InBodyScan[]> {
        return this.inBodyScanRepository.find({
    ...
    async findLatest(clientId: string, tenantId: string): Promise<InBodyScan | null> {
    ...
    async calculateProgress(clientId: string, tenantId: string) {
        ...
        return {
            ...
            changes: {
                ...
                bodyFatPercentage: parseFloat((latest.bodyFatPercentage - first.bodyFatPercentage).toFixed(2)),
                skeletalMuscleMass: parseFloat((latest.skeletalMuscleMass - first.skeletalMuscleMass).toFixed(2)),
                weightPercent: parseFloat(((latest.weight - first.weight) / first.weight * 100).toFixed(2)),
                bodyFatPercent: parseFloat(((latest.bodyFatPercentage - first.bodyFatPercentage) / first.bodyFatPercentage * 100).toFixed(2)),
                muscleMassPercent: parseFloat(((latest.skeletalMuscleMass - first.skeletalMuscleMass) / first.skeletalMuscleMass * 100).toFixed(2)),
            },
