...
export class WaitingListService {
  ...
  async create(
    createDto: CreateWaitingListEntryDto,
    tenantId: string,
    ...
    const status = createDto.requiresApproval
  ...
  async findAll(tenantId: string, filters?: any): Promise<WaitingListEntry[]> {
    const query = this.waitingListRepo
    ...
    if (filters?.studioId) {
      query.andWhere('entry.studioId = :studioId', {
  ...
  async update(
    id: string,
    updateDto: UpdateWaitingListEntryDto,
    tenantId: string,
  ): Promise<WaitingListEntry> {
