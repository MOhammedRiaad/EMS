export const FEATURE_PERMISSION_MAP: Record<string, string[]> = {
    ...
    // Coach
    'core.coaches': ['coach.read', 'coach.create', 'coach.update', 'coach.delete', 'coach.list'],
 ...
 */
export function isPermissionAllowed(permissionKey: string, enabledFeatures: string[]): boolean {
    for (const [featureKey, restrictedPatterns] of Object.entries(FEATURE_PERMISSION_MAP)) {
