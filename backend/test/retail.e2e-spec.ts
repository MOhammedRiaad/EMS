...
describe('RetailModule (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const clientRes = await request(app.getHttpServer())
            .post('/clients')
            .set('Authorization', `Bearer ${jwtToken}`)
            .send({ firstName: 'Shopper', lastName: 'One', email: 'shopper@test.com', phone: '5551234' });
    ...
    it('/retail/sales (POST) - process sale', async () => {
        ...
        expect(Number(res.body.totalAmount)).toBe(11.00); // 5.50 * 2
    });
    ...
    it('/retail/transactions (GET) - POS Report', async () => {
        ...
        expect(Number(tx.amount)).toBe(11.00);
    });
