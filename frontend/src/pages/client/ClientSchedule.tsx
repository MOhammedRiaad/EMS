            ...
            <div className="space-y-3">
                {filteredSessions.length === 0 ? (
                    ...
                    filteredSessions.map(session => (
                        <div key={session.id} className={`bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-800 relative overflow-hidden ${session.status === 'cancelled' ? 'opacity-75 bg-gray-50 dark:bg-slate-800' : ''}`}>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm p-6 space-y-4 shadow-xl">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={closeReviewModal} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                                <X size={24} />
                            </button>
                        </div>
