    ...
    return (
        <div className="max-w-lg mx-auto pb-20 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">{profile.firstName} {profile.lastName}</h1>
                    <div className="text-gray-500 dark:text-gray-400">{profile.email}</div>
                    <div className="text-gray-500 dark:text-gray-400 text-sm">{profile.phone}</div>
                </div>
            </div>
                ...
                <div className="space-y-3">
                    {history.length === 0 ? (
                        ...
                        history.map((session: any) => (
                            <div key={session.id} className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
                                ...
                                {session.notes && (
                                    <div className="bg-gray-50 dark:bg-slate-800 p-2 rounded text-sm text-gray-600 dark:text-gray-300 italic">
                                        "{session.notes}"
                                    </div>
                                )}
                            </div>
